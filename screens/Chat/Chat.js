...
export default class Chat extends Component {
	...
	constructor(props) {
		...
		this.state = {
			loading: true,
			// bookingId: this.props.navigation.getParam('bookingId', 89), // TODO DEBUG
	...
	getMessages() {
		this.auth.getLocalUserData().then(res => {
			this.setState({
				  ...
				  this.api.getMessages(this.state.bookingId).then(res => {
	...
	onSend(messages = []) {
		...
		if (messages.length && messages[(messages.length - 1)].text) {
			...
			this.api.postMessage(text, this.state.bookingId).then(res => {
				console.log(res);
				this.getMessages();
			}).catch(err => {
				console.log(err);
